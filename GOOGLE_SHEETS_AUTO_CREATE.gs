// Google Apps Script - Auto-creates spreadsheet on first sync
// Deploy this as STANDALONE script (not bound to any sheet)
...
function doPost(e) {
  try {
    ...
    if (data.action === 'ping') {
      ...
      return respond({ success: true, message: 'Connected', sheetUrl: ss.getUrl() });
    }
...
function getOrCreateSpreadsheet() {
  ...
  if (spreadsheetId) {
    try {
      ...
      // Spreadsheet deleted or no access - create new one
    }
  }

  // Create new spreadsheet
  var ss = SpreadsheetApp.create('ON POINT CRM - Jobs Data');
  props.setProperty('SPREADSHEET_ID', ss.getId());

  // Share with anyone with link (view only)
  ss.addViewer('anyone@example.com');
