...
const DB = (() => {
  ...
  // A "ghost" is a corrupt cache entry: no id, or no customer name AND no real
  // status. The server has no such row, so a normal delete can never remove it —
  // it must be pruned locally or it renders forever as "Unknown Customer".
  function _isGhostJob(j) {
    if (!j || typeof j !== 'object') return true;
    if (!j.jobId) return true;
    const hasName = typeof j.customerName === 'string' && j.customerName.trim() !== '';
    const hasStatus = typeof j.status === 'string' && j.status.trim() !== '' && j.status !== 'undefined';
    return !hasName && !hasStatus;
  }

  // Remove every ghost from the local cache. Returns how many were removed.
  function purgeGhostJobs() {
    const all = Storage.getJobs() || [];
    const clean = all.filter(j => !_isGhostJob(j));
    const removed = all.length - clean.length;
    if (removed > 0) Storage.saveJobs(clean);
    return removed;
  }
  ...
  async function _syncJobsDown() {
    try {
      ...
      Storage.saveJobs(merged.filter(j => !_isGhostJob(j)));
    } catch (e) {
  ...
  function getJobs()                 { return (Storage.getJobs() || []).filter(j => !_isGhostJob(j)); }
  ...
  async function saveJob(job) {
    ...
    // AUTO-SYNC to Google Sheets on every change (create / close / lost / edit).
    // Fire-and-forget: never block or fail the save if the sheet is unreachable.
    try {
      if (typeof SyncManager !== 'undefined' && SyncManager.syncJob) {
        Promise.resolve(SyncManager.syncJob(job.jobId)).catch(() => {});
      }
    } catch (e) { /* sheets sync is best-effort */ }
