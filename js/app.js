...
const App = (() => {
  ...
  async function _onAuthenticated() {
    ...
    try { if (DB.purgeGhostJobs) DB.purgeGhostJobs(); } catch(e){}
    _loadSettingsForm();
    renderDashboard();
    renderJobList();
    _renderTechSelector();
    ...
    _profilesChannel = DB.subscribeToProfiles(async (payload) => {
      ...
      if (currentUser && changedUserId === currentUser.id) {
        ...
        try { if (_state.currentView === 'dashboard') renderDashboard(); else if (_state.currentView === 'jobs') renderJobList(); } catch(e){}
  ...
  function _applyRoleUI() {
    ...
    if (moreBalance) moreBalance.classList.toggle('hidden', !Auth.isAdmin());
  ...
  function navigate(viewName, opts = {}) {
    ...
    // Role guard: only admins can access Reports/Balance
    if (viewName === 'balance' && !Auth.isAdmin()) {
      showToast('Reports are admin-only', 'error');
      navigate('dashboard');
      return;
    }
  ...
  function renderDashboard() {
    ...
    _setText('count-estimate', counts.estimate);
    ...
    if (Auth.isTechOrContractor()) {
      ...
      const recent = jobs.filter(j => j.status !== 'paid' && j.status !== 'lost').slice(0, 3);
      if (recent.length === 0) {
  ...
  function _statusBucket(status) {
    ...
    if (status === 'follow_up') return 'estimate';
    return 'open'; // new, scheduled, in_progress
  }
  ...
  function _jobCardHTML(job) {
    ...
    const waBtn = Auth.isAdminOrDisp()
      ? `<button class="wa-btn" onclick="event.stopPropagation();App.openDispatchModal('${job.jobId}')" title="Dispatch to Tech">&#128172;</button>`
      : '';
  ...
  async function parseLead() {
    ...
    try {
      ...
      const _tw = _normTimeWindow(j.scheduledTime);
      set('f-time', _tw);
  ...
  function _buildJobDetailHTML(job) {
    ...
    const dispatchBtn = Auth.isAdminOrDisp()
      ? `<button class="detail-action-primary-btn dab-primary-dispatch" onclick="event.stopPropagation();App.openDispatchModal('${job.jobId}')">
    ...
    const copyBtn = Auth.isAdminOrDisp()
      ? `<button class="detail-action-primary-btn dab-primary-copy" onclick="event.stopPropagation();App.openDispatchModal('${job.jobId}','copy')">
    ...
    return `
      ...
      <div class="detail-action-bar">
        ...
        ${Auth.isAdminOrDisp() && (job.status === 'paid' || job.status === 'closed') ? `<button class="detail-action-btn dab-green" onclick="App.openClosingSheet('${job.jobId}')"><span class="dab-icon">&#129534;</span><span class="dab-label">Closing</span></button>` : ''}
  ...
  // ── Month picker: tap the date → grid with job counts → tap a day to jump ──
  let _calPickerMonth = null;

  function openCalendarPicker() {
    _calPickerMonth = new Date(_state.calendarDate.getFullYear(), _state.calendarDate.getMonth(), 1);
    const ex = document.getElementById('cal-picker-overlay');
    if (ex) ex.remove();
    const ov = document.createElement('div');
    ov.id = 'cal-picker-overlay';
    ov.className = 'cal-picker-overlay';
    ov.addEventListener('click', (e) => { if (e.target === ov) ov.remove(); });
    ov.innerHTML = '<div class="cal-picker-sheet" id="cal-picker-sheet"></div>';
    document.body.appendChild(ov);
    _renderCalendarPicker();
  }

  function calendarPickerShiftMonth(delta) {
    if (!_calPickerMonth) return;
    _calPickerMonth = new Date(_calPickerMonth.getFullYear(), _calPickerMonth.getMonth() + delta, 1);
    _renderCalendarPicker();
  }

  function _renderCalendarPicker() {
    const sheet = document.getElementById('cal-picker-sheet');
    if (!sheet || !_calPickerMonth) return;
    ...
    const selD = _state.calendarDate;
    const selStr = `${selD.getFullYear()}-${String(selD.getMonth()+1).padStart(2,'0')}-${String(selD.getDate()).padStart(2,'0')}`;

    // count jobs per day for this month
    const counts = {};
    (DB.getJobs() || []).forEach(j => {
      if (!j.scheduledDate) return;
      if (_state.calendarTechFilter && j.assignedTechId !== _state.calendarTechFilter) return;
      if (j.status === 'lost') return;
      counts[j.scheduledDate] = (counts[j.scheduledDate] || 0) + 1;
    });
    ...
    for (let d = 1; d <= daysInMonth; d++) {
      ...
      if (ds === selStr) cls.push('cal-selected');
      cells += `<button class="${cls.join(' ')}" onclick="App.calendarPickDay('${ds}')">`
             + `<span class="cal-day-num">${d}</span>`
             + (n ? `<span class="cal-day-count">${n}</span>` : '<span class="cal-day-count cal-day-count-empty"></span>')
             + `</button>`;
    }
    ...
    sheet.innerHTML =
      ...
      +   '<button class="cal-picker-arrow" onclick="App.calendarPickerShiftMonth(-1)">&#8249;</button>'
      +   '<div class="cal-picker-month">' + monthName + '</div>'
      +   '<button class="cal-picker-arrow" onclick="App.calendarPickerShiftMonth(1)">&#8250;</button>'
      + '</div>'
      + '<div class="cal-dow">' + ['S','M','T','W','T','F','S'].map(x => '<div>' + x + '</div>').join('') + '</div>'
      + '<div class="cal-grid">' + cells + '</div>'
      + '<button class="cal-picker-today" onclick="App.calendarPickDay(\'' + todayStr + '\')">Jump to Today</button>';
  }

  function calendarPickDay(dateStr) {
    const parts = String(dateStr).split('-').map(Number);
    if (parts.length === 3) {
      _state.calendarDate = new Date(parts[0], parts[1] - 1, parts[2]);
      renderCalendar();
    }
  ...
  // Dispatch/copy messages: always name the day, and call out Today / Tomorrow
  // so a tech reading it on their phone knows immediately when to show up.
  function _formatDispatchDate(dateStr) {
    ...
    try {
      ...
      if (diffDays === 0) return 'TODAY (' + weekday + ') - ' + pretty;
      if (diffDays === 1) return 'TOMORROW (' + weekday + ') - ' + pretty;
  ...
  // Remove phone-like number sequences (used for 50% dispatch privacy)
  function _scrubPhones(t){
    return String(t||'').replace(/(\+?\d[\d().\-\s]{7,}\d)/g, '[hidden]');
  }

  // ── CLOSING SUMMARY — what the tech owes the company, the split, and Zelle ──
  function _buildClosingMsg(job) {
    const settings = DB.getSettings();
    const tech = job.assignedTechId ? (settings.technicians || []).find(t => t.id === job.assignedTechId) : null;
    const calc = _jobCalc(job) || {};
    const ref = (job.jobId || '').slice(-6).toUpperCase();
    const money = (n) => '$' + (Number(n) || 0).toFixed(2);

    const techCut = Number(calc.techPayout) || 0;
    const companyCut = (Number(calc.ownerPayout) || 0) + (Number(calc.contractorFee) || 0);
    const collected = Number(calc.jobTotal) || 0;
    const parts = Number(calc.partsCost) || 0;
    // If the tech collected the money, they owe the company its share.
    const owed = companyCut;

    const lines = ['*JOB CLOSING*'];
    if (ref) lines.push('Ref #' + ref);
    lines.push('');
    ...
    if (job.scheduledDate) lines.push(_formatDispatchDate(job.scheduledDate));
    lines.push('');

    lines.push('*Collected*');
    lines.push('Total: ' + money(collected));
    if (parts > 0) {
      lines.push('Parts: -' + money(parts));
      lines.push('Net: ' + money(collected - parts));
    }
    if (job.paymentMethod) lines.push('Paid by: ' + String(job.paymentMethod).toUpperCase());
    lines.push('');

    lines.push('*Split*');
    lines.push((tech ? tech.name : 'Tech') + ' (' + (Number(calc.techPercent) || 0) + '%): ' + money(techCut));
    lines.push('Company: ' + money(companyCut));
    lines.push('');

    lines.push('*TECH OWES COMPANY: ' + money(owed) + '*');

    const ownerZelle = settings.ownerZelle || '';
    if (ownerZelle) {
      lines.push('');
      lines.push('*Send via Zelle to:*');
      lines.push(ownerZelle);
    }
    return lines.join('\n');
  }

  // Closing sheet: send on WhatsApp or copy the same text
  function openClosingSheet(jobId) {
    ...
    const calc = _jobCalc(job) || {};
    const owed = (Number(calc.ownerPayout) || 0) + (Number(calc.contractorFee) || 0);
    sheet.innerHTML =
      '<div class="dispatch-choice-handle"></div>' +
      '<div class="dispatch-choice-title">Closing — ' + _esc(job.customerName || '') + '</div>' +
      '<div class="dispatch-choice-sub">Tech owes company <b>$' + owed.toFixed(2) + '</b></div>' +
      '<button class="dispatch-choice-btn" id="cl-wa"><b>WhatsApp</b><span>Send the closing summary</span></button>' +
      '<button class="dispatch-choice-btn" id="cl-copy"><b>Copy</b><span>Copy the summary to paste anywhere</span></button>' +
      '<button class="dispatch-choice-cancel" id="cl-cancel">Cancel</button>';
    ov.appendChild(sheet);
    document.body.appendChild(sheet.parentElement === ov ? ov : ov);
    ...
    sheet.querySelector('#cl-copy').addEventListener('click', () => {
      ...
      if (navigator.clipboard && navigator.clipboard.writeText) {
        navigator.clipboard.writeText(msg)
          .then(() => showToast('Closing summary copied', 'success'))
          .catch(() => showToast('Copy failed', 'error'));
      } else showToast('Clipboard not available', 'error');
    });
    sheet.querySelector('#cl-cancel').addEventListener('click', () => {
      const o = document.getElementById('dispatch-choice-overlay'); if (o) o.remove();
    });
  }
  ...
  // Dispatch choice: 100% (full) vs 50% (city+zip only, no phone) — for subcontractors
  function openDispatchModal(jobId, action) {
    if (!Auth.isAdminOrDisp()) return;
    const job = DB.getJobById(jobId);
    if (!job) { showToast('Job not found', 'error'); return; }
    const ex = document.getElementById('dispatch-choice-overlay');
    ...
    ov.appendChild(sheet);
    document.body.appendChild(ov);
    sheet.querySelector('#dc-full').addEventListener('click', () => _doDispatch(jobId, 'full', action));
    sheet.querySelector('#dc-half').addEventListener('click', () => _doDispatch(jobId, 'half', action));
    sheet.querySelector('#dc-cancel').addEventListener('click', () => ov.remove());
  }
  function _doDispatch(jobId, mode, action) {
    const o = document.getElementById('dispatch-choice-overlay');
    if (o) o.remove();
    if (action === 'copy') copyJobDetails(jobId, mode); else openWhatsApp(jobId, mode);
  }

  function openWhatsApp(jobId, mode) {
    if (!Auth.isAdminOrDisp()) return;
    const job = DB.getJobById(jobId);
    if (!job) { showToast('Job not found', 'error'); return; }
  ...
  function copyJobDetails(jobId, mode) {
    if (!Auth.isAdminOrDisp()) return;
    const job = DB.getJobById(jobId);
    if (!job) { showToast('Job not found', 'error'); return; }
  ...
  // ══════════════════════════════════════════════════════════════
  // POINTY — On Point AI assistant (chat). One Haiku call per message.
  // Proposes actions; NOTHING writes to the DB until the operator taps Confirm.
  // ══════════════════════════════════════════════════════════════
  let _pointyPending = {};
  let _pointyHistory = [];
  let _pointySeq = 0;

  // Normalize a free-text time (e.g. "8-9 am", "12:00 PM - 2 PM") into the app's
  // canonical 24h window "HH-HH". Returns '' if no time is present.
  function _normTimeWindow(str){
    if(!str) return '';
  ...
  function _pointyEl() { return document.getElementById('pointy-messages'); }

  function _pointyFmt(t){ return _esc(String(t||'')).replace(/\*\*(.+?)\*\*/g,'<b>$1</b>').replace(/(^|<br>|\n)\s*[\-\u2022]\s*/g,'$1&bull; ').replace(/\n/g,'<br>'); }

  function _pointyAppend(role, html) {
    const box = _pointyEl(); if (!box) return null;
    const wrap = document.createElement('div');
    wrap.className = 'pointy-msg ' + (role === 'user' ? 'pointy-user' : 'pointy-bot');
    wrap.innerHTML = role === 'user'
      ? `<div class="pointy-bubble">${_esc(html)}</div>`
      : `<div class="pointy-avatar">&#10022;</div><div class="pointy-bubble">${html}</div>`;
    box.appendChild(wrap);
    box.scrollTop = box.scrollHeight;
    return wrap;
  }

  async function pointySend() {
    const input = document.getElementById('pointy-input');
    const msg = input?.value?.trim();
    if (!msg) return;
    input.value = ''; input.style.height = 'auto';
    _pointyAppend('user', msg);
    _pointyHistory.push({ role:'user', text: msg });
    const typing = _pointyAppend('bot', '<span class="pointy-typing">Pointy is thinking&hellip;</span>');
    ...
    try {
      ...
      const res = await fetch('/api/ai-assistant', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + token },
        body: JSON.stringify({ message: msg, jobs, history: _pointyHistory.slice(-6) })
      });
      const data = await res.json().catch(() => ({}));
      typing?.remove();
      if (!res.ok) throw new Error(data.error || 'Pointy had a problem');
      _pointyRenderAction(data);
    } catch (e) {
      typing?.remove();
      _pointyAppend('bot', _esc(e.message || 'Something went wrong'));
    }
  }
  ...
  function _pointyRenderAction(data) {
    ...
    if (a === 'chat' || !a) { _pointyHistory.push({ role:'assistant', text: (data.reply||'') });
      _pointyAppend('bot', _pointyFmt(data.reply || "I can add a job, close one, mark it lost, or answer questions about your jobs.")); return; }

    const id = ++_pointySeq;
    _pointyPending[id] = { action: a, params: p };
    let title = '', body = '';
    ...
    if (a === 'add_job') {
      ...
      title = 'Close job';
      body = `<b>${_esc(p.customerName || (job&&job.customerName) || '?')}</b><br>` +
        `Total <b>$${Number(p.jobTotal||0).toFixed(2)}</b>` +
        (p.partsCost ? ` · Parts $${Number(p.partsCost).toFixed(2)}` : '') +
        ` · ${_esc(p.paymentMethod || 'cash')}` +
      ...
      title = 'Assign tech';
      body = '<b>' + _esc(p.customerName||'?') + '</b> &rarr; ' + _esc(p.techName||'?') +
        (!p.jobId ? '<br><span class="pointy-warn">&#9888; Couldn\'t find that job.</span>' : '');
    } else if (a === 'reopen_job') {
      title = 'Reopen job';
      body = '<b>' + _esc(p.customerName||'?') + '</b><br>Back to Open.' +
        (!p.jobId ? '<br><span class="pointy-warn">&#9888; Couldn\'t find that job.</span>' : '');
    } else if (a === 'delete_job') {
      title = '&#9888; Delete job';
      body = '<b>' + _esc(p.customerName||'?') + '</b><br><span class="pointy-warn">This permanently deletes the job.</span>' +
        (!p.jobId ? '<br><span class="pointy-warn">&#9888; Couldn\'t find that job.</span>' : '');
    } else if (a === 'add_note') {
      title = 'Add note';
      body = '<b>' + _esc(p.customerName||'?') + '</b><br><i>' + _esc(p.note||'') + '</i>' +
        (!p.jobId ? '<br><span class="pointy-warn">&#9888; Couldn\'t find that job.</span>' : '');
    } else if (a === 'add_tech') {
      title = 'Add technician';
      body = '<b>' + _esc(p.name||'?') + '</b>' + (p.percent!=null ? ' &middot; ' + Number(p.percent) + '% payout' : '');
    } else if (a === 'add_company') {
      title = 'Add company';
      body = '<b>' + _esc(p.name||'?') + '</b>' + (p.contractorPercent!=null ? ' &middot; takes ' + Number(p.contractorPercent) + '%' : '');
    } else if (a === 'mark_estimate') {
      title = 'Mark as Estimate';
      body = '<b>' + _esc(p.customerName||'?') + '</b><br>Moves to the Estimate list for follow-up.' +
        (!p.jobId ? '<br><span class="pointy-warn">&#9888; Couldn\'t find that job.</span>' : '');
      ...
      body = '<b>' + _esc(p.customerName||'?') + '</b><br>' + (ch || 'No changes detected') +
        (!p.jobId ? '<br><span class="pointy-warn">&#9888; Couldn\'t find that job.</span>' : '');
    } else if (a === 'sync_sheets') {
      title = 'Sync to Google Sheets';
      body = 'Push all jobs to your connected sheet.';
    } else if (a === 'bulk_action') {
      const filter = p.filter || 'open';
      const ex = (p.excludeNames||[]).map(n => String(n).toLowerCase().trim()).filter(Boolean);
      const ids = p.includeJobIds||[];
      let match = (DB.getJobs()||[]).filter(j => {
        if (ids.length) return ids.includes(j.jobId);
        if (filter === 'all') return j.status !== 'lost' && j.status !== 'paid';
        if (filter === 'open') return ['new','scheduled','in_progress'].includes(j.status);
        if (filter === 'estimate') return j.status === 'follow_up';
        return j.status === filter;
      });
      if (ex.length) match = match.filter(j => !ex.some(n => (j.customerName||'').toLowerCase().includes(n)));
      title = '&#9888; Bulk action';
      _pointyPending[id].bulkJobs = match.map(j => j.jobId);
      body = `You asked to <b>${p.op==='mark_estimate'?'mark ESTIMATE':'mark LOST'}</b> <b>${match.length}</b> job(s)${ex.length?' (excluding '+_esc((p.excludeNames||[]).join(', '))+')':''}:<br>` +
        match.slice(0,15).map(j => '· ' + _esc(j.customerName)).join('<br>') +
        (match.length>15 ? `<br>…and ${match.length-15} more` : '') +
        `<br><span class="pointy-warn">This changes ${match.length} jobs at once. Are you sure?</span>`;
    }

    const NEEDS_ID = ['close_job','mark_lost','mark_estimate','update_job','dispatch_job','assign_tech','reopen_job','delete_job','add_note'];
    const canDo = (a === 'add_job') || (a === 'bulk_action') || (a === 'sync_sheets') || (a === 'add_tech') || (a === 'add_company') || (NEEDS_ID.includes(a) && p.jobId);
    const card = `<div class="pointy-card"><div class="pointy-card-t">${title}</div><div class="pointy-card-b">${body}</div>` +
      (canDo ? `<div class="pointy-card-actions"><button class="pointy-btn-cancel" onclick="App.pointyCancel(${id})">Cancel</button><button class="pointy-btn-do" onclick="App.pointyDo(${id})">${a==='bulk_action'?'Yes, do it':'Confirm'}</button></div>` : '') +
      `</div>`;
    _pointyAppend('bot', card);
  }

  function pointyCancel(id) { delete _pointyPending[id]; _pointyAppend('bot', 'Okay, cancelled.'); }

  const _POINTY_ADMIN_ONLY = ['add_tech','add_company','delete_job','bulk_action'];

  async function pointyDo(id) {
    const pend = _pointyPending[id]; if (!pend) return;
    if (_POINTY_ADMIN_ONLY.includes(pend.action) && !Auth.isAdmin()) {
      delete _pointyPending[id];
      _pointyAppend('bot', 'That one is owner-only.');
      return;
    }
    delete _pointyPending[id];
    const { action: a, params: p } = pend;
    try {
      if (a === 'add_job') {
        const settings = DB.getSettings();
        ...
        const job = {
          ...
          estimatedTotal: 0, jobTotal: 0, partsCost: 0, taxAmount: 0, techPayout: 0, ownerPayout: 0, contractorFee: 0,
          paymentMethod: 'cash', photos: [], syncStatus: 'pending'
        };
        await DB.saveJob(job);
        ...
        const calc = PayoutEngine.calculate({
          ...
          techPercent: parseFloat(job.techPercent)||0, contractorPct: parseFloat(job.contractorPct)||0,
          taxOption: 'none', isSelfAssigned: job.isSelfAssigned,
          taxRateNY: settings.taxRateNY, taxRateNJ: settings.taxRateNJ
        });
        job.jobTotal = calc.jobTotal; job.partsCost = calc.partsCost;
        job.techPayout = calc.techPayout; job.ownerPayout = calc.ownerPayout; job.contractorFee = calc.contractorFee;
        job.paymentMethod = p.paymentMethod || 'cash'; job.status = 'paid';
        job.paidAt = new Date().toISOString(); job.closedBy = Auth.getUser()?.id || null;
        await DB.saveJob(job);
        try { SyncManager.queueJob(job.jobId); } catch(e){}
        _pointyAppend('bot', `&#9989; Closed <b>${_esc(job.customerName)}</b> — you keep <b>$${calc.ownerPayout.toFixed(2)}</b>, tech $${calc.techPayout.toFixed(2)}.`);
      } else if (a === 'dispatch_job') {
        openWhatsApp(p.jobId, (p.detail == 50) ? 'half' : 'full');
        _pointyAppend('bot', '&#9989; Opened WhatsApp (' + ((p.detail==50)?'50%':'100%') + ').');
      } else if (a === 'assign_tech') {
        const job = DB.getJobById(p.jobId); if (!job) throw new Error('Job not found');
        const st = DB.getSettings();
        const want = String(p.techName||'').toLowerCase();
        const t = (st.technicians||[]).find(x => (x.name||'').toLowerCase().includes(want));
        if (!t) throw new Error('No technician named ' + (p.techName||''));
        job.assignedTechId = t.id; job.assignedTechName = t.name;
        job.isSelfAssigned = !!t.isOwner; job.techPercent = t.percent || 0;
        await DB.saveJob(job);
        try { SyncManager.queueJob(job.jobId); } catch(e){}
        _pointyAppend('bot', '&#9989; <b>' + _esc(job.customerName) + '</b> assigned to ' + _esc(t.name) + '.');
      } else if (a === 'reopen_job') {
        const job = DB.getJobById(p.jobId); if (!job) throw new Error('Job not found');
        job.status = job.scheduledDate ? 'scheduled' : 'new';
        job.paidAt = null;
        await DB.saveJob(job);
        _pointyAppend('bot', '&#9989; <b>' + _esc(job.customerName) + '</b> reopened.');
      } else if (a === 'delete_job') {
        const job = DB.getJobById(p.jobId);
        if (!job) {
          // The row is not on the server (a corrupt local "ghost"). Purge it.
          const n = DB.purgeGhostJobs ? DB.purgeGhostJobs() : 0;
          try { if (_state.currentView === 'jobs') renderJobList(); else renderDashboard(); } catch(e){}
          _pointyAppend('bot', n > 0
            ? '&#9989; Removed ' + n + ' broken job entr' + (n === 1 ? 'y' : 'ies') + '.'
            : 'That job is already gone.');
          return;
        }
        const nm = job.customerName;
        await DB.deleteJob(p.jobId);
        _pointyAppend('bot', '&#9989; Deleted <b>' + _esc(nm) + '</b>.');
      } else if (a === 'add_note') {
        const job = DB.getJobById(p.jobId); if (!job) throw new Error('Job not found');
        job.notes = ((job.notes||'') + (job.notes ? ' | ' : '') + p.note).trim();
        await DB.saveJob(job);
        _pointyAppend('bot', '&#9989; Note added to <b>' + _esc(job.customerName) + '</b>.');
      } else if (a === 'add_tech') {
        const st = DB.getSettings();
        const techs = [...(st.technicians||[])];
        techs.push({ id: DB.generateId(), name: p.name, phone:'', color:'#3B82F6',
          percent: Number(p.percent)||0, zelle:'', zipCodes:[], isOwner:false, isUserAccount:false });
        DB.updateSettingsCache({ ...st, technicians: techs });
        const sess = await SupabaseClient.auth.getSession();
        const r = await fetch('/api/save-technicians', { method:'POST',
          headers:{'Content-Type':'application/json','Authorization':'Bearer '+sess.data.session.access_token},
          body: JSON.stringify({ technicians: techs.filter(t=>!t.isUserAccount) }) });
        if (!r.ok) throw new Error('Save failed');
        _pointyAppend('bot', '&#9989; Added technician <b>' + _esc(p.name) + '</b>.');
      } else if (a === 'add_company') {
        const st = DB.getSettings();
        const ls = [...(st.leadSources||[])];
        ls.push({ id: DB.generateId(), name: p.name, contractorPercent: Number(p.contractorPercent)||0 });
        await DB.saveSettings({ ...st, leadSources: ls });
        _pointyAppend('bot', '&#9989; Added company <b>' + _esc(p.name) + '</b>.');
      } else if (a === 'mark_estimate') {
        const job = DB.getJobById(p.jobId); if (!job) throw new Error('Job not found');
        job.status = 'follow_up'; await DB.saveJob(job);
        _pointyAppend('bot', '&#9989; <b>' + _esc(job.customerName) + '</b> moved to Estimates.');
      } else if (a === 'update_job') {
        const job = DB.getJobById(p.jobId); if (!job) throw new Error('Job not found');
        if (p.scheduledDate) job.scheduledDate = p.scheduledDate;
        if (p.scheduledTime) job.scheduledTime = _normTimeWindow(p.scheduledTime);
        if (p.address) job.address = p.address;
        if (p.city) job.city = p.city;
        if (p.state) job.state = p.state;
        if (p.zip) job.zip = p.zip;
        ...
        await DB.saveJob(job);
        try { SyncManager.queueJob(job.jobId); } catch(e){}
        _pointyAppend('bot', '&#9989; Updated <b>' + _esc(job.customerName) + '</b>.');
      } else if (a === 'sync_sheets') {
        _pointyAppend('bot', 'Syncing to Google Sheets&hellip;');
        try { await App.syncAll(); _pointyAppend('bot', '&#9989; Synced.'); }
        catch(e) { _pointyAppend('bot', 'Sync failed: ' + _esc(e.message||'error')); }
      } else if (a === 'bulk_action') {
      ...
      _pointyAppend('bot', _esc('Could not do that: ' + (e.message||'error')));
    }
  }
